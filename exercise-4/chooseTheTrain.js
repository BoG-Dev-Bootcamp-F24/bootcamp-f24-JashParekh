...
function updateLineColor(arrivals) {
  const blueLineArrivals = arrivals.filter(arrival => arrival.LINE === 'BLUE');
  
  const pinkLineArrivals = blueLineArrivals.map(arrival => {
    return { ...arrival, LINE: 'PINK' };
  });

  return pinkLineArrivals;
}
